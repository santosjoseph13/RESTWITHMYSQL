...
Examinee.createAccount = function (newExaminee, resultofproc) {
    ...
    bcrypt.hash(newExaminee.password, saltRounds, function (err, hash) {
        console.log(newExaminee)
        sql.query("INSERT INTO examinee (email,password,fname,mname,lname,job_title) VALUES (?,?,?,?,?,?)", [newExaminee.email,hash,newExaminee.fname,newExaminee.mname,newExaminee.lname,newExaminee.job_title], function (errorfromquery, resultfromquery) {
...
Examinee.getScore = (examinee_id) => {
    return new Promise((resolve, reject) => {
        const ACTION = '[getScore]';
        logger.log('info', `${TAG}${ACTION}[SELECT * FROM answers WHERE examinee_id=${examinee_id}]`);
        sql.query("SELECT * FROM answer WHERE examinee_id=?", examinee_id, function (err, res) {
            if (err) {
...
Examinee.getEssay = (examinee_id) => {
    return new Promise((resolve, reject) => {
        const ACTION = '[getScore]';
        logger.log('info', `${TAG}${ACTION}[SELECT * FROM answers WHERE examinee_id=${examinee_id}]`);
        sql.query("SELECT * FROM essay WHERE examinee_id=?", examinee_id, function (err, res) {
            if (err) {
...
Examinee.submitAnswer = (exam_id, body) => {
    console.log("examid", exam_id)
    console.log("answer", body.answer)
    return new Promise((resolve, reject) => {
        const ACTION = '[submtAnswer]';
        logger.log('info', `${TAG}${ACTION}[SELECT * FROM books WHERE id=?]`);
        sql.query("INSERT INTO  answer (`examinee_id`,`exam_id`,`answer`,`exam_duration`) VALUES (2,?,?,?)", [exam_id, body[0].answer, body[0].exam_duration], function (err, res) {
            if (err) {
                console.log(err)
                return reject(err);
            }
...
Examinee.finishedExam = (exam_id, body) => {
    console.log("examid", exam_id)
    console.log("answer", body.date)
    return new Promise((resolve, reject) => {
        const ACTION = '[submtAnswer]';
        logger.log('info', `${TAG}${ACTION}[SELECT * FROM books WHERE id=?]`);
        sql.query("INSERT INTO  examinee (`examinee_id`,`exam_finished`) VALUES (3,?)", [ body.date], function (err, res) {
            if (err) {
                console.log(err)
                return reject(err);
            }
